...
public class JupiterBeanDefinitionParser
  implements BeanDefinitionParser
{
  ...
  public BeanDefinition parse(Element element, ParserContext parserContext)
  {
    ...
    if (beanClass == JupiterSpringClient.class)
      return parseJupiterClient(element, parserContext);
    if (beanClass == JupiterSpringProviderBean.class)
      return parseJupiterProvider(element, parserContext);
    if (beanClass == JupiterSpringConsumerBean.class) {
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if ((item instanceof Element)) {
        ...
        if ("property".equals(localName)) {
          ...
          addPropertyReferenceArray(def, (Element)item, ProviderInterceptor.class.getName(), "globalProviderInterceptors", false);
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if ((item instanceof Element)) {
        ...
        if ("property".equals(localName)) {
          ...
          addPropertyReferenceArray(def, (Element)item, ConsumerInterceptor.class.getName(), "globalConsumerInterceptors", false);
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if ((item instanceof Element)) {
        ...
        if ("property".equals(localName)) {
          ...
          addPropertyReferenceArray(def, (Element)item, ProviderInterceptor.class.getName(), "providerInterceptors", false);
    ...
    for (int i = 0; i < childNodes.getLength(); i++) {
      ...
      if ((item instanceof Element)) {
        ...
        if ("property".equals(localName)) {
          ...
          addPropertyReferenceArray(def, (Element)item, ConsumerInterceptor.class.getName(), "consumerInterceptors", false);
          ...
          for (int j = 0; j < configList.getLength(); j++) {
            Node configItem = configList.item(j);
            if (((configItem instanceof Element)) && 
              ("methodSpecial".equals(configItem.getLocalName()))) {
              ...
              MethodSpecialConfig config = MethodSpecialConfig.of(methodName).timeoutMillis(Long.parseLong(timeoutMillis)).strategy(ClusterStrategyConfig.of(clusterStrategy, failoverRetries));
  ...
  private void parseNetOption(Node configItem, List<Pair<JOption<Object>, String>> parentOptions, List<Pair<JOption<Object>, String>> childOptions)
  {
    ...
    if (("parentOption".equals(localName)) && (parentOptions != null)) {
      for (JOption<?> op : JOption.ALL_OPTIONS) {
        ...
        if (Strings.isNotBlank(value)) {
          parentOptions.add(Pair.of(op, value));
        }
      }
      ...
      for (JOption<?> op : JOption.ALL_OPTIONS) {
        String value = ((Element)configItem).getAttribute(op.name());
        if (Strings.isNotBlank(value)) {
          childOptions.add(Pair.of(op, value));
        }
      }
    }
  }
  ...
  private static void addPropertyReferenceArray(RootBeanDefinition definition, Element element, String elementTypeName, String propertyName, boolean required)
  {
